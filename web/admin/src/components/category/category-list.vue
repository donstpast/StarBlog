  ...
  <div>
    <el-card class="cardBox">
      ...
      <el-row :gutter="20">
        <el-col :span="4">
          <!-- 新增按钮 -->
          <el-button type="primary" @click="addCategoryFormVisible = true"> 新增分类 </el-button>
        </el-col>
      </el-row>
      <!-- 分类列表 -->
      <el-table stripe style="width: 100%" class="categoryTable" :data="categoryList">
        ...
        <el-table-column label="操作" align="center">
          <template #default="scope">
            <!-- 编辑按钮 -->
            <el-button size="small" @click="editCategoryBtn(scope.row)">Edit </el-button>
            <!-- 删除按钮 -->
    ...
    <!-- Form -->
    <el-dialog
      v-model="addCategoryFormVisible"
      title="新增分类"
      width="30%"
      @close="clearForm(addCategoryRef)"
    >
      <el-form
    ...
    <!-- Form -->
    <el-dialog
      v-model="editCategoryFormVisible"
      title="编辑分类"
      width="30%"
      @close="clearForm(editCategoryRef)"
    >
      <el-form
      ...
      >
        <el-form-item label="分类名" :label-width="formLabelWidth" prop="name">
          <el-input v-model="editCategoryForm.name" :clearable="true" autocomplete="off" />
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button
            type="primary"
            @click="
              editCategoryFormVisible;
              editCategory(categoryList, editCategoryRef)
            "
          >
            Confirm
          </el-button>
...
const deleteCategory = async (categoryList: CategoryList) => {
  try {
    ...
    const confirmResult = await ElMessageBox.confirm('确认要删除该分类信息么？', '提示', {
